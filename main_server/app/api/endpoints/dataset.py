...
async def add_usage_event(
    ...
    return {"message": "Event added successfully" if event_added else "Event not added"}
...
async def get_events_statistic(
    ...
    repository = DatasetUsageHistoryRepository(session)
    return await repository.get_events_statistic(client_request.hostname, client_request.dataset_name)
